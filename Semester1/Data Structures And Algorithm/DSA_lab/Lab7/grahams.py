...
class Point:   #store x and y coordinates of pts
	def __init__(self, x = None, y = None):
		self.x = x
		self.y = y

p0 = Point(0, 0)    #global point


def nextToTop(S):   #to find next to top in stack
	return S[-2]


def distSq(p1, p2):  # find square of distance between p1 and p2
...
def convexHull(points, n):  # Prints convex hull of a set of n points.
	ymin = points[0].y     #find bottommost pt
	min = 0
	for i in range(1, n):
		y = points[i].y
		...
		if ((y < ymin) or (ymin == y and points[i].x < points[min].x)):
			ymin = points[i].y
			min = i
	points[0], points[min] = points[min], points[0]  	# Place the bottom-most point at first position
